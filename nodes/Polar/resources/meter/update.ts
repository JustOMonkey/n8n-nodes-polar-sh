	...
	},
	{
		...
		routing: {
			send: {
				...
				property: 'filter',
				value:
					'={{ $value ? JSON.parse($value) : (($parameter["filterClauses"].clause || []).length ? { conjunction: $parameter["filterConjunction"], clauses: $parameter["filterClauses"].clause.map((c) => ({ property: c.property, operator: c.operator, value: (c.value === "true" ? true : (c.value === "false" ? false : (c.value !== "" && !isNaN(Number(c.value)) ? Number(c.value) : c.value))) })) } : undefined) }}',
			},
		},
	},
	{
